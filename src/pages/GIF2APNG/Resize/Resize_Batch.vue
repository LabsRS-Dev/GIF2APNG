...
export default {
  ...
  methods: {
            ...
            [
              {
                ...
                filePath:
                  ...
                  _prx,
                fileSize: '27.5MB',
                fileDimensions: { data: { width: 250, height: 150 } }
              },
              {
                ...
                fileSize: '10.7MB',
                fileDimensions: { data: { width: 512, height: 512 } }
              }
            ],
            function (ele) {
      ...
      if (data.success) {
        ...
        _.each(imageFiles, (fileObj, dinx) => {
          ...
          if (BS.b$.App.checkPathIsFile(fileObj.filePath)) {
            var dimensions = {}
            BS.b$.Binary.getImageFileInfo({ path: fileObj.filePath }, function (info) {
          ...
          _.each(that.taskList, (taskObj, index) => {
            that.__abi__start_ResizeGifTask(
              taskObj.id,
              {
                ...
                enableIncludeMinImage: that.onHandleTheTask && true,
    ...
    ValidateWidthNumber () {
      var that = this
      if (that.onBtnLock) {
        that.inputHeightAll = that.$t(
          'pages.resize.dialog-config-change.input-lock'
        )
        that.inputWidthAll = ''
      }
    },
    ...
    onPreviewFile (item) {
      ...
      cdg.confirmButtonText = that.$t(
        'pages.resize.dialog-config-preview.btnConfirm'
      )
