    ...
    if (!hasInited) {
      ...
      Transfer['pageResizeTest'] = {
        updateTaskProcessInfo: task_id => {
          ...
          var progressTask = setInterval(() => {
            const dateInfo = Math.round(Math.random() * 10)
            console.log(dateInfo)
            if (progressInterval < 100) {
              progressInterval += 10
              Transfer.trigger('TestRunGif2apngTask', {
                data: {
  ...
  methods: {
    ...
    onToolReplayBtnClick () {
      ...
      if (that.enableCustomRatio) {
        ...
        if (
          that.defaultCurWidth < clientWidth * 0.4 &&
          that.defaultCurHeight < clientHeight * 0.6
        ) {
          ...
          const width = clientHeight * 0.6 / that.defaultCurHeight * that.defaultCurWidth
          ...
          const height = clientWidth * 0.4 / that.defaultCurWidth * that.defaultCurHeight
          ...
          const cliWidth = that.defaultCurWidth / (clientWidth * 0.4)
          const cliHeight = that.defaultCurHeight / (clientHeight * 0.6)
          if (cliWidth >= cliHeight) {
            const endWidth = clientWidth * 0.4
            const endHeight = that.defaultCurHeight / cliWidth
            $('.sliderImage').css('width', endWidth)
            $('.sliderImage').css('height', endHeight)
            $('.imageBlur').css('width', endWidth)
            $('.imageBlur').css('height', endHeight)
          } else {
            const endWidth = that.defaultCurWidth / cliHeight
            const endHeight = clientHeight * 0.6
            $('.sliderImage').css('width', endWidth)
            $('.sliderImage').css('height', endHeight)
            $('.imageBlur').css('width', endWidth)
            $('.imageBlur').css('height', endHeight)
      ...
      BS.b$.importFiles(
        ...
        function () {
          ...
          var img = [
            ...
            {
              path: 'http://p2.so.qhimgs1.com/t01a4723418f8de871e.gif',
              width: 585,
              height: 240
            },
            {
          ...
          const ele = {
            ...
            filePath: img[index].path,
            fileSize: '27.5MB',
            fileDimensions: {
              data: { width: img[index].width, height: img[index].height }
            }
          }
          ...
          $('.sliderRange').css('background-size', that.percentage + '% 100%')
          $('.sliderCustomRange').css(
            'background-size',
            that.percentageCustom / 2 + '% 100%'
          )
          return
        },
        function (data) {
    ...
    onBtnDoClick () {
      ...
      if (
        that.percentage === 100 &&
        that.enableMaintainAspectRatio &&
        that.enableCustomRatio === false
      ) {
        BS.b$.Notice.alert({
    ...
    startDo () {
      ...
      if (that.taskList.length > 0) {
        _.each(that.taskList, (taskObj, index) => {
          const fileName = BS.b$.App.getFileName(taskObj.path)
          const filePath = that.lastOutputPath + '/' + fileName
          if (
            that.enableOverWriteOutput === false &&
            BS.b$.App.checkPathIsExist(filePath)
          ) {
            ...
            cdg.confirmButtonText = that.$t(
              'pages.resize.dialog-confirm-isExist.btnConfirm'
            )
            cdg.denyButtonText = that.$t(
              'pages.resize.dialog-confirm-isExist.btnDeny'
            )
              ...
              that.__abi__start_ResizeGifTask(
                ...
                data => {
                  if (data.infoType === 'type_calltask_log') {
                    ...
                    $('.image__single__top__message').css(
                      'background-color',
                      'rgba(0,0,0,0)'
                    )
                    that.__updateInfoWithGif2apngTask(taskObj.id, {
    ...
    __abi__start_ResizeGifTask (taskID, config, handler = data => {}) {
      ...
      that.__updateTaskObj(taskID, { fixOutDir: _config.dest }, taskObj => {
    ...
    __abi__cancel_Gif2apngTask (taskID, handler = data => {}) {
      ...
      var curTaskObj = taskID2taskObj[taskID]
      ...
      if (that.enableCustomRatio) {
        ...
        if (value >= 16) {
          if (value > clientWidth * 0.2) {
            ...
            if (that.defaultImageScale < scale) {
              ...
              const scaling = 1 / that.imageScale
        ...
        if (value >= 16) {
          if (that.enableMaintainAspectRatio === true) {
            ...
            that.curWidth = value
            that.curHeight =
              value / that.defaultCurWidth * that.defaultCurHeight
            that.inputHeight = Math.round(that.curHeight)
            that.percentage = Math.round(value / that.defaultCurWidth * 100)
            if (
              that.curWidth < clientWidth * 0.4 &&
              that.curHeight < clientHeight * 0.6
            ) {
  ...
  watch: {
      ...
      if (that.inputActive === 10 || that.inputActive === 100) {
        ...
        if (that.onChangeTestFile) {
          if (sidesDifference > 0 && that.enableMaintainAspectRatio === true) {
            ...
            that.curWidth =
              that.curWidth + that.defaultCurWidth / 100 * sidesDifference
            that.curHeight =
              that.curHeight + that.defaultCurHeight / 100 * sidesDifference
            that.inputWidth = Math.round(that.curWidth)
            that.inputHeight = Math.round(that.curHeight)
            if (
              that.curWidth <= clientWidth * 0.4 &&
              ...
              var height =
                clientWidth * 0.4 / that.defaultCurWidth * that.defaultCurHeight
              $('.sliderImage').css('width', clientWidth * 0.4)
              $('.sliderImage').css('height', height)
              $('.imageBlur').css('width', clientWidth * 0.4)
              $('.imageBlur').css('height', height)
              ...
              var width =
                clientHeight *
                0.6 /
                that.defaultCurHeight *
                that.defaultCurWidth
              $('.sliderImage').css('width', width)
              $('.sliderImage').css('height', clientHeight * 0.6)
              $('.imageBlur').css('width', width)
              $('.imageBlur').css('height', clientHeight * 0.6)
            ...
            that.curWidth =
              that.curWidth -
              that.defaultCurWidth / 100 * absoluteSidesDifference
            that.curHeight =
              that.curHeight -
              that.defaultCurHeight / 100 * absoluteSidesDifference
            that.inputWidth = Math.round(that.curWidth)
            that.inputHeight = Math.round(that.curHeight)
            if (
              that.curWidth < clientWidth * 0.4 &&
      ...
      if (that.onChangeTestFile) {
        if (sidesDifference > 0) {
          ...
          if (that.isCheckPercent === 1) {
            ...
            that.customWidth =
              that.customWidth + that.firstCustomWidth / 100 * sidesDifference
            that.customHeight =
              that.customHeight + that.firstCustomHeight / 100 * sidesDifference
            that.inputWidth = Math.round(that.customWidth)
            that.inputHeight = Math.round(that.customHeight)
          } else {
            that.customWidth =
          ...
          if (that.isCheckPercent === 1) {
            ...
            that.customWidth =
              that.customWidth -
              that.firstCustomWidth / 100 * absoluteSidesDifference
            that.customHeight =
              that.customHeight -
              that.firstCustomHeight / 100 * absoluteSidesDifference
            that.inputWidth = Math.round(that.customWidth)
            that.inputHeight = Math.round(that.customHeight)
          } else {
            that.customWidth =
    ...
    enableMaintainAspectRatio (val, oldVal) {
      ...
      if (that.WidthFocus) {
        ...
        that.curHeight = Math.round(
          that.defaultCurHeight * (that.finalInputWidth / that.defaultCurWidth)
        )
        that.inputWidth = that.finalInputWidth
        that.inputHeight = Math.round(
          that.defaultCurHeight * (that.finalInputWidth / that.defaultCurWidth)
        )
        that.percentage = Math.round(
          100 * (that.finalInputWidth / that.defaultCurWidth)
        )
        ...
        that.curWidth = Math.round(
          that.defaultCurWidth * (that.finalInputHeight / that.defaultCurHeight)
        )
        that.curHeight = that.finalInputHeight
        that.inputWidth = Math.round(
          that.defaultCurWidth * (that.finalInputHeight / that.defaultCurHeight)
        )
        that.inputHeight = that.finalInputHeight
        that.percentage = Math.round(
          100 * (that.finalInputHeight / that.defaultCurHeight)
        )
    ...
    enableCustomRatio (val, oldVal) {
      ...
      if (val) {
        ...
        if (that.isCheckNumber === 1) {
          ...
          $('.sliderCustomImage').css('height', that.customHeight)
          if (
            that.customWidth <= clientWidth * 0.2 &&
            that.customHeight <= clientHeight * 0.3
          ) {
            that.imageScale = 1
            ...
            const width = that.customWidth / (clientWidth * 0.2)
            const height = that.customHeight / (clientHeight * 0.3)
            if (width >= height) {
