            ...
            <div class="children__router__content__topRank__body">
                <div class="children__router__content__topRank__body__top">
                    <div class="children__router__content__topRank__body__top__content" v-for="(el,index) in item.top" :key="el.id">
                        <img class="children__router__content__topRank__body__top__icon" :src="el.thumb">
                ...
                <div class="children__router__content__topRank__body__bottom">
                    <div class="children__router__content__topRank__body__bottom__content" v-for="(ele,index) in item.bottom" :key="ele.id">
                        ...
                        <img class="children__router__content__topRank__body__bottom__icon" :src="ele.thumb">
...
export default {
  ...
  methods: {
        ...
        [
          {
            fileName: 'gif001',
            fileThumb:
              'http://bbs.uc.cn/data/attachment/forum/month_0807/20080706_d162e4910d0a301fb136NkPw7m39KEOc.gif',
            fileBrowse: 80,
            fileDownload: 3,
            fileShare: 2,
            fileCollect: 98
          },
          {
            fileName: 'gif002',
            fileThumb:
              'http://i5.bbs.fd.zol-img.com.cn/t_s240x240/g3/M0A/08/05/Cg-4V1Celt6ICLZpAAUPtUKK9n4AABhHwP51s8ABQ_N875.gif',
            fileBrowse: 98,
            fileDownload: 12,
            fileShare: 11,
            fileCollect: 89
          },
          {
            fileName: 'gif003',
            fileThumb:
              'http://worldcup.sznews.com/humor/attachement/gif/site3/20140721/4487fcd7fc661537068e36.gif',
            fileBrowse: 66,
            fileDownload: 33,
            fileShare: 32,
            fileCollect: 68
          },
          {
            fileName: 'gif004',
            fileThumb:
              'http://ww3.sinaimg.cn/bmiddle/6331b8ebgw1eglth1kwv4g205c074u0x.gif',
            fileBrowse: 55,
            fileDownload: 44,
            fileShare: 43,
            fileCollect: 57
          },
          {
            fileName: 'gif005',
            fileThumb:
              'http://images6.fanpop.com/image/photos/35300000/cal-cal-calum-hood-35346616-180-240.gif',
            fileBrowse: 74,
            fileDownload: 28,
            fileShare: 27,
            fileCollect: 73
          },
          {
            fileName: 'gif006',
            fileThumb:
              'https://upload.wikimedia.org/wikipedia/commons/thumb/2/2a/Candle-light-animated.gif/180px-Candle-light-animated.gif',
            fileBrowse: 12,
            fileDownload: 80,
            fileShare: 79,
            fileCollect: 21
          },
          {
            fileName: 'gif007',
            fileThumb:
              'http://img0.pconline.com.cn/pconline/1404/08/4571216_2012043023524670421_thumb.gif',
            fileBrowse: 46,
            fileDownload: 54,
            fileShare: 53,
            fileCollect: 47
          },
          {
            fileName: 'gif008',
            fileThumb:
              'http://img.zcool.cn/community/01d579578e4bed0000018c1b7c42ab.gif',
            fileBrowse: 18,
            fileDownload: 82,
            fileShare: 81,
            fileCollect: 19
    ...
    drawTopRankInfo () {
      var that = this
      const browseTopList = _.sortBy(topRankList, 'browse')
        .reverse()
        .slice(0, 3) // 图片文件浏览次数前三位
      const browseBottomList = _.sortBy(topRankList, 'browse')
        .reverse()
        .slice(3, 8) // 图片文件浏览次数三到八
      const downloadTopList = _.sortBy(topRankList, 'download')
        .reverse()
        .slice(0, 3) // 图片文件下载次数前三位
      const downloadBottomList = _.sortBy(topRankList, 'download')
        .reverse()
        .slice(3, 8) // 图片文件下载次数三到八
      const shareTopList = _.sortBy(topRankList, 'share')
        .reverse()
        .slice(0, 3) // 图片文件分享次数前三位
      const shareBottomList = _.sortBy(topRankList, 'share')
        .reverse()
        .slice(3, 8) // 图片文件分享次数三到八
      const collectTopList = _.sortBy(topRankList, 'collect')
        .reverse()
        .slice(0, 3) // 图片文件收藏次数前三位
      const collectBottomList = _.sortBy(topRankList, 'collect')
        .reverse()
        .slice(3, 8) // 图片文件收藏次数三到八
      _.each(
        [
          {
            fileName: '浏览榜单',
            fileTop: browseTopList,
            fileBottom: browseBottomList,
            fileProps: 'browse'
          },
          {
            fileName: '下载榜单',
            fileTop: downloadTopList,
            fileBottom: downloadBottomList,
            fileProps: 'download'
          },
          {
            fileName: '分享榜单',
            fileTop: shareTopList,
            fileBottom: shareBottomList,
            fileProps: 'share'
          },
          {
            fileName: '收藏榜单',
            fileTop: collectTopList,
            fileBottom: collectBottomList,
            fileProps: 'collect'
          }
        ],
        function (ele) {
          const allRankObj = new Model(
            ele.fileName,
            ele.fileTop,
            ele.fileBottom,
            ele.fileProps
